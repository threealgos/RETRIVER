...
// Constructor from signed 64-bit integer
Int::Int(int64_t i64) {
    if (i64 < 0) {
        CLEARFF(); // Set all bits to 1
    } else {
        CLEAR(); // Set all bits to 0
    }
    bits64[0] = i64; // Assign to the least significant part
}

// Constructor from unsigned 64-bit integer
Int::Int(uint64_t u64) {
    CLEAR();
    bits64[0] = u64; // Assign to the least significant part
}
...
// Member function defined in `Int.cpp`
void Int::SetFromInt(int value) {
    bits64[0] = value; // Assign value to the least significant part
    bits64[1] = 0;     // Reset other parts
    bits64[2] = 0;
    bits64[3] = 0;
}

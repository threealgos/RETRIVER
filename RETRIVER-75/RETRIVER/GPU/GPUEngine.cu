...
enum TameType {
    ...
    // Optionally, more types can be added here
};
    ...
    for (int b = 0; b < nbBlock; b++) {
        for (int g = 0; g < GPU_GRP_SIZE; g++) {
            for (int t = 0; t < nbThreadPerGroup; t++) {
                ...
                // Simplified offset: use a running total based on index
                Int offset;
                offset.SetFromInt(idx); // Set offset using a helper function to convert 'idx' to 'Int'

                inputKangarooPinned[g * strideSize + t + 8 * nbThreadPerGroup] = offset.bits64[0];
                inputKangarooPinned[g * strideSize + t + 9 * nbThreadPerGroup] = offset.bits64[1];

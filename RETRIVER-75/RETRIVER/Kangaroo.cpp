...
bool Kangaroo::CollisionCheck(Int* d1, uint32_t type1, Int* d2, uint32_t type2) {
    if (type1 == type2) {
        ...
        if (type1 >= TAME && type1 <= TAME50) {
            Td.Set(d1);
            Wd.Set(d2);
            TameType = type1;
            WildType = type2;
        } else if (type2 >= TAME && type2 <= TAME50) {
            Td.Set(d2);
            Wd.Set(d1);
            TameType = type2;
            WildType = type1;
            ...
            // Loop to handle wild types from WILD4 to WILD50
            for (int i = 4; i <= 50; i++) {
        ...
        bool endOfSearch = CheckKey(Td, Wd, 0) || CheckKey(Td, Wd, 1) || 
...
void Kangaroo::CreateHerd(int nbKangaroo, Int *px, Int *py, Int *d, int firstType, bool lock) {
    ...
    for (uint64_t j = 0; j < nbKangaroo; j++) {
        ...
        if (typeIndex == WILD) {
            ...
            // Create a temporary Int object to hold the value
            Int multiplier(static_cast<int64_t>(typeIndex - WILD + 1)); // Create a named Int object
            wildRange.ModMul(&multiplier); // Use the named object
